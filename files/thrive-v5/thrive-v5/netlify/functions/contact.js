...
exports.handler = async (event) => {
  ...
  if (payload.website)                     return response(202, { ok:true }); // honeypot
  const webhookUrl = process.env.CONTACT_WEBHOOK_URL;
  if (!webhookUrl) return response(503, { error:'Contact backend is not configured' });
  const r = await fetch(webhookUrl, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ type:'contact', submittedAt:new Date().toISOString(), name, email, interest:type, message }) });
  if (!r.ok) return response(502, { error:'Notification delivery failed' });
