...
exports.handler = async (event) => {
  ...
  if (!webhookUrl) return response(503, { error:'Donor backend is not configured' });
  const privateRecord = { type:'donor_confirmation', donorId, submittedAt:new Date().toISOString(), donor:{ name, email }, donation:{ amount, currency:'NGN' }, acknowledgement:{ preference:acknowledgement, displayName } };
  const r = await fetch(webhookUrl, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(privateRecord) });
  if (!r.ok) return response(502, { error:'Donor notification delivery failed' });
